...
import com.example.elasticService.databaseService.repositories.ConnectionRepository;
...
public class SQLiteConnectionService {
    ...
    public SQLiteConnectionService(ConnectionRepository connectionRepository) {
        this.connectionRepository = connectionRepository;
    }

    // Create a connection by user_id
    public  List<SQLiteConnectionEntity> createConnection(Integer userId, String type, SQLiteConnectionDetailsHelper SQLiteConnectionDetailsHelper) throws Exception {
        // Create the Connection object
        SQLiteConnectionEntity newConnection = new SQLiteConnectionEntity(userId, type, SQLiteConnectionDetailsHelper);

//        System.out.println("final coonection " + newConnection.toString());
        ...
        connectionRepository.saveConnectionManually(newConnection.getConnectionString(), type, userId);
...
//        List<ConnectionEntity> connections = userConnectionMap.getOrDefault(userId, Collections.emptyList());
        ...
        List<SQLiteConnectionEntity> connections = connectionRepository.findByUserId(userId);
